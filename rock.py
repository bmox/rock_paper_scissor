...
class handDetector():
    def __init__(self, mode=False, maxHands=2, detectionCon=0.5, trackCon=0.5,csv_path="gesture_train.csv"):
        ...
        self.hands = self.mpHands.Hands(self.static_image_mode,self.maxHands,
...
def main():
    ...
    cap.set(3,wCam)
    cap.set(4,hCam)
    detector = handDetector(csv_path="gesture_train.csv")
    while cap.isOpened():
        ret, img = cap.read()
        if not ret:
            continue
